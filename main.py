...
def main():
    ...
    for x in data:
        ...
        deactivate_this_collection("Space Cowboy")
        ...
        for obj in objectIterator(bpy.data.collections["Space Cowboy"]):
            try:
                obj.select_set(True)
            except:
                pass
        ...
        # activate_collection("Final_Environments")

        # renders the image then clears the output collection
        # bpy.ops.wm.save_as_mainfile(
        #     filepath=
        #     "/home/gangadhar/Documents/Work/Upwork/AssetPositionCombination/freescape-master/scene_render.blend"
        # )

        render(output_dir="./out", output_filename=str(counter))

        scale_obj(bpy.data.objects["Character"], 0.001)
